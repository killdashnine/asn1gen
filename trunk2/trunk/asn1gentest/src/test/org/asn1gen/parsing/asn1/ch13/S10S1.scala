...
package test.org.asn1gen.parsing.asn1.ch13 {
  class TestS10S1 extends TestCase {
    ...
    @Test def test_1() {
      val text = """
        MoreCompact ::= OCTET STRING (
          CONTAINING MyType ENCODED BY {
    ...
    @Test def test_1_1_1_1_1_1() {
      ...
      """
      parse(contentsConstraint, text) match {
        case Success(_, _) => ()
        case x => fail("Parse failure: " + x)
      }
    }
